...
namespace FishyFacepunch
{
    public class FishyFacepunch : Transport
    {
        ...
        private int[] _mtus;
        /// <summary>
        /// EWYF: true once SteamClient.Init has thrown, so it is not retried every call.
        /// </summary>
        private bool _steamInitFailed;
        /// <summary>
...
#if !UNITY_SERVER
            // EWYF: guarded. Upstream calls SteamClient.Init unconditionally and Facepunch throws
            // when the Steam client is not running, which took the whole process down during
            // NetworkManager.Awake. Every headless test in this project runs without Steam, so a
            // missing Steam client has to be a declined transport, not a crash. See #13.
        ...
        /// <summary>
        /// EWYF: brings Steam up if it is not already, and reports whether it is usable.
        ///
        /// Safe to call repeatedly and safe to call when Steam is not installed or not running: the
        /// first failure is logged once as a warning and every later call returns false without
        /// retrying, so a build launched from a terminal script logs one line instead of one per
        /// frame. Facepunch throws rather than returning a code, hence the catch.
        /// </summary>
        public bool TryInitializeSteam()
        {
            if (SteamClient.IsValid) return true;
            if (_steamInitFailed) return false;
            ...
            {
                _steamInitFailed = true;
                Debug.LogWarning($"[FishyFacepunch] Steam app {_steamAppID} unavailable, this "
                                 + $"transport is disabled: {e.Message}");
                return false;
            }
...
#if !UNITY_SERVER
            // EWYF: a warning, not an error. With Multipass this transport sits next to Tugboat and
            // declining is the expected outcome on a machine with no Steam client. See #13.
            if (!TryInitializeSteam())
            {
                Debug.LogWarning("[FishyFacepunch] Steam not available; server not started on this "
                                 + "transport.");
                return false;
            }
        ...
        {
            // EWYF: see StartServer.
            if (!TryInitializeSteam())
            {
                Debug.LogWarning("[FishyFacepunch] Steam not available; client not started on this "
                                 + "transport.");
                return false;
            }
